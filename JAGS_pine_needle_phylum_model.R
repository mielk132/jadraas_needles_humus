...
model{ 
  ...
  for(i in 1:nobs) {
    ...
    log(muk[i]) <- alpha + eblock[block[i]] + b_basid*basid[i,1] + b_asco*asco[i,1] 
    }
  ...
  #K <- mean(k[]) # k for all bags
  
  for(k in 1:length(bm_basid_pred)){
   log(k_bm_pred[k]) <- alpha + b_basid*bm_basid_pred[k] #+ b_asco*bm_asco_pred[k] 
     #+ b_bm2*bm_pred[k]^2
 }
